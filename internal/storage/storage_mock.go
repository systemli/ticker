...
import (
	...
	util "github.com/systemli/ticker/internal/api/util"
)
...
// FindMessagesByTicker provides a mock function with given fields: ticker, pagination
func (_m *MockTickerStorage) FindMessagesByTicker(ticker Ticker, pagination util.Pagination) ([]Message, error) {
	ret := _m.Called(ticker, pagination)

	var r0 []Message
	if rf, ok := ret.Get(0).(func(Ticker, util.Pagination) []Message); ok {
		r0 = rf(ticker, pagination)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).([]Message)
		}
	...
	var r1 error
	if rf, ok := ret.Get(1).(func(Ticker, util.Pagination) error); ok {
		r1 = rf(ticker, pagination)
	} else {
		r1 = ret.Error(1)
	}
